...
func (h *Handler) RegisterHandlers(e *gin.Engine) {
	...
	systemCacl := api.Group("/system_calcs", h.AuthoMiddleware())
	...
	systemCacl.PUT("/:id/status_formed", h.RoleValidateMiddleware(UserRole), h.UpdateSystemCalcStatusToFormed)
	systemCacl.PUT("/:id/status", h.RoleValidateMiddleware(ModeratorRole), h.UpdateSystemCalcStatusModerator)
	...
	users.GET("/me", h.AuthoMiddleware(), h.GetUser)
	users.PUT("/me", h.AuthoMiddleware(), h.UpdateUser)
