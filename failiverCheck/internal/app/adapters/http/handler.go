...
func (h *Handler) RegisterSysteemCalcToComponentsHandlers(router *gin.RouterGroup) {
	users := router.Group("", h.ModeratorValidateMiddleware())
	users.DELETE("/system_calcs_to_components/", h.DeleteComponentsToSystemCac)
	users.PUT("/system_calcs_to_components/", h.UpdateComponentsToSystemCac)
}
